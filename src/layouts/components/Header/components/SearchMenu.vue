...
const menuList = computed(() =>
	authStore.flatMenuListGet.filter(item => !item.meta.isHide && item.meta.type !== enum_system_router_meta_type.button)
);
...
const filterNodeMethod = (queryString: string) => {
	return (restaurant: Auth.MenuProps) => {
		return (
...
// 点击菜单跳转
const handleClickMenu = (menuItem: Auth.MenuProps | Record<string, any>) => {
	searchMenu.value = "";
