...
<script setup lang="ts" name="AUploadImgs">
import { ref, computed, inject, watch } from "vue";
import { uploadImg } from "@/api/modules/upload";
import { ElNotification, formContextKey, formItemContextKey } from "element-plus";
import type { UploadProps, UploadFile, UploadUserFile, UploadRequestOptions } from "element-plus";

export interface AUploadImgsProps {
...
const props = withDefaults(defineProps<AUploadImgsProps>(), {
	api: undefined,
	modelValue: () => [],
	drag: true,
	disabled: false,
...
const uploadFileList = ref<UploadUserFile[]>(props.modelValue);

// 监听 props.fileList 列表默认值改变
watch(
	() => props.modelValue,
	(n: UploadUserFile[]) => {
		uploadFileList.value = n;
	}
);
...
const handleRemove = (file: UploadFile) => {
	uploadFileList.value = uploadFileList.value.filter(item => item.url !== file.url || item.name !== file.name);
	emit("update:modelValue", uploadFileList.value);
};
