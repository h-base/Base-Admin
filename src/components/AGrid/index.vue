...
<script setup lang="ts" name="AGrid">
import {
	ref,
	watch,
	useSlots,
...
const findIndex = () => {
	...
	slots?.forEach((slot: any) => {
		if (typeof slot.type === "object" && slot.type.name === "AGridItem" && slot.props?.suffix !== undefined)
			suffix = slot;
	...
	try {
		...
		fields.reduce((prev = 0, current, index) => {
			...
			if ((prev as number) > props.collapsedRows * colsComputed.value - suffixCols) {
...
const style = computed(() => {
	return {
		display: "grid",
		gridGap: gapComputed.value,
		gridTemplateColumns: `repeat(${colsComputed.value}, minmax(0, 1fr))`
	};
});
